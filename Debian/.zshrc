...
pullscript() {
  ...
  # Use fzf to select the file
  # Includes a preview window using batcat (aliased as cat in your config)
  local selected_file=$(ls "$SCRIPT_DIR" | fzf \
    --prompt="Select script to pull > " \
    --height=40% \
    --reverse \
    --preview "batcat --color=always --style=numbers $SCRIPT_DIR/{}")

  # If a file was selected (didn't press ESC)
  if [[ -n "$selected_file" ]]; then
    cp "$SCRIPT_DIR/$selected_file" .
    chmod +x "$selected_file"
    echo "Successfully pulled '$selected_file' and set executable permissions."
  else
